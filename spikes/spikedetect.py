...
cf = gaussian(x, mu=0, sigma=0.05)
cf = cf/np.max(cf)


peaks = scipy.signal.find_peaks(tod, prominence=np.max(tod[65000:90000])*2.5)
...
for factor in factors:
    cs = scipy.signal.convolve(tod, cf*factor, mode='same')
    peaks = scipy.signal.find_peaks(cs, prominence=np.max(cs[65000:90000])*2)
    peak_heights = cs[peaks[0]]
    std_noise = np.std(cs[65000:90000])
    sn = peak_heights/std_noise
    
    print('%d         %.2f          %.2f          %.2f         %.2f' %(factor, sn[0], sn[1], sn[2], sn[3]))
