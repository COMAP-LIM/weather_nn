...
def read_file(output_folder, n, line):
    ...
    try:
        with h5py.File(path + 'comp_' + filename, 'r') as hdf:
...
def create_dataset_parallel(file_with_good_samples, file_with_bad_samples, good_folder, bad_folder, n=400):
    """  
    Preprocess data and write dataset to files.
    Args: 
        file_with_good_samples (string): Text file with good samples.
        file_with_bad_samples  (string): Text file with bad samples.
        good_folder            (string): Folder where preprocessed good samples is written to. 
        bad_folder             (string): Folder where preprocessed bad samples is written to. 
        n                         (int): Number of samples preprocessed.
    Returns: 
        ps (ndarray): Scaled power spectrum. 
    """

    textfile_bad = open(file_with_bad_samples, 'r')
    textfile_good = open(file_with_good_samples, 'r')

    lines_good = textfile_good.readlines()[:n]
    lines_bad = textfile_bad.readlines()[:n]


    read_file_bad = partial(read_file, bad_folder, 0)
    read_file_good = partial(read_file, good_folder, 0)
...
if __name__ == '__main__':
    ...
    file_with_good_samples = 'data/good_subsequences_ALL.txt'
    file_with_bad_samples  = 'data/bad_subsequences_ALL.txt'

    good_folder = 'data/good_data/'
    bad_folder  = 'data/bad_data/'

    create_dataset_parallel(file_with_good_samples, file_with_bad_samples, good_folder, bad_folder)
