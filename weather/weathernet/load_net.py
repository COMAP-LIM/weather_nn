...
def subsequencegen(filename):
    ...
    return np.array(sequences), subtimes, tod_mean, time
...
for f in files:
    ...
    for i in range(len(predictions)):
        file_subseq.write('%d    %d    %.4f \n' %(int(obsid), i+1, predictions[i][1]))

    file_obsid = open('weather_list_obsid.txt', 'a')
    file_obsid.write('%d    %.4f    %.4f \n' %(int(obsid), max(predictions[:,1]), np.median(predictions[:,1])))
